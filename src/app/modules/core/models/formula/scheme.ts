    ...
    */
    ...
    static evalte(x, env = {}) {
        ...
        if (x === undefined)
            ...
            var objectToBeMatched = Scheme.evalte(x[1], env);



            for (let i = 2; i < x.length; i++) {

                var val = Scheme.unify(objectToBeMatched, x[i][0]);
...
static parser(s: string) {
    ...
    if (tokens.length > 0) {
        ...
        Scheme.debugerrormessage("In the expression '" + s + "', I understood the beginning of it but I did not understand the end '" + rest + "'")
    }
...
static tokenize(s: string): [string] {
    ...
    let tab = s.split("\n");
    tab = tab.filter(function (line) { return !Scheme.trim(line).startsWith(";"); });
    s = tab.join("\n");

    let result = Scheme.trim(s.replace(/'/g, "' ")
        .replace(/`/g, "` ")
        .replace(/,/g, ", ")
        .replace(/\(/g, " ( ")
        .replace(/\)/g, " ) "))
